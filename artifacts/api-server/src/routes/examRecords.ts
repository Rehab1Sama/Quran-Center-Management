...
import { db, examRecordsTable, studentsTable, usersTable, recordsTable, circlesTable } from "@workspace/db";
...
router.get("/volunteer/near-completion", authenticate, async (req, res): Promise<void> => {
  ...
  if (role === "student") {
    ...
    const totalPages = Math.round(sRecords.reduce((a, r) => a + (r.memorizePages ?? 0), 0) * 2) / 2;
